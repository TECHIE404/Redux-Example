...
import { decrease, increase, divide, multiply, incrementByAmount } from '../state/countReducer';

function MainPage() {

    const count=useSelector((state)=> state.counter.value);
    const dispatch = useDispatch()
    const [increaseByAmount, setIncreaseByAmount]=useState(0);
    ...
    <main className={styles.main}>
        ...
        <div className={styles.grid}>
          ...
          >
            ...
            <input type="number" placeholder="Amount" value={incrementByAmount} 
            onChange={(e)=>setIncreaseByAmount(e.target.value)}/>
            <button className="btn btn-primary" 
            onClick={()=>dispatch(incrementByAmount(Number(incrementByAmount) || 0))}>Apply</button>
          </a>
          <a
            className={styles.card}
          >
